...
exports.test = function (web3, accounts, token) {
  describe("operatorSend", function () {
    ...
    it(
      `should let ${utils.formatAccount(accounts[3])} ` +
      `send 1 ${token.symbol} from ${utils.formatAccount(accounts[1])} ` +
      ...
      async function () {
        ...
        await token.contract.methods.authorizeOperator(accounts[3]).send({
          from: accounts[1],
          gas: 300000
        });
        ...
        await utils.assertBalance(web3, token, accounts[2], 11);
        await eventsCalled;
      }
    );

    it(
      `should not let ${utils.formatAccount(accounts[3])} send from ` +
      `${utils.formatAccount(accounts[1])} (not operator)`,
      async function () {
        await utils.assertTotalSupply(
          web3,
          token,
    ...
    it(
      ...
      async function () {
        ...
        await utils.assertBalance(web3, token, accounts[3], 6);
        await utils.assertBalance(web3, token, accounts[2], 14);
        await eventsCalled;
      }
    );
    ...
    //     await token.contract.methods
    //       .operatorSend(
    //         accounts[3], accounts[2], utils.tokenUnit('4'), '0x', '0x')
    //       .send({
    //         gas: 300000,
    //         from: accounts[3]
    //       });
