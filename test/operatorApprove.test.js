...
contract('SelfToken', function (accounts) {
  ...
  it("should allow an operator to approve tokens for a token holder", async function () {
    ...
    await selfToken.mint(user1, 100, "", { from: owner });

    // authorize operator1 as user1's operator
    await selfToken.authorizeOperator(operator1, { from: user1 });

    // operator1 approve user1's 10 tokens to user2
    assert.ok(await selfToken.operatorApprove(user1, user2, 10, { from: operator1 }));
    ...
    assert.ok(await selfToken.transferFrom(user1, user3, 10, { from: user2 }));
