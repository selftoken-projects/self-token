...
const DelegatedTransferOperator = artifacts.require("DelegatedTransferOperatorV5GasOptimized");
...
console.log("_web3.version", _web3.version);
...
contract('DelegatedTransferOperatorV5GasOptimized', function ([
  owner,
  alice,
  bob,
  charlie,
  ...
  it('should delegated transfer if signedWithPrefix', async function () {
    const nonce = 1;
    const from = alice;
    const to = bob;
    const delegate = charlie;
    ...
    const delegationHash = hashedTightPacked(delegation);

    const delegationHashWithPrifexHash = hashedTightPacked([
      Buffer.from("\x19Ethereum Signed Message:\n32", 'utf8'),
      delegationHash,
    ]);

    // vrs.v: number, vrs.r: Buffer, vrs.s: Buffer
    const vrs = await ethUtil.ecsign(delegationHashWithPrifexHash, alicePrivateKey);
    // const sig = await ethUtil.toRpcSig(vrs.v, vrs.r, vrs.s);
  ...
  it('should delegated transfer if signedWithPrefix, using web3.utils.soliditySha3', async function () {
    const nonce = 1;
    const from = alice;
    const to = bob;
    const delegate = charlie;
    ...
    // string "0x..."
    const delegationHash = _web3.utils.soliditySha3(
      { t: "address", v: operator.address },
      { t: "address", v: to },
      { t: "address", v: delegate },
      { t: "uint256", v: amount.toString() },
      { t: "uint256", v: fee.toString() },
      { t: "uint256", v: nonce.toString() },
      { t: "bytes", v: userData },
    );

    const delegationHashWithPrifexHash = formattedAddress(
      _web3.utils.soliditySha3(
