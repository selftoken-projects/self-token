...
contract('SelfToken', function (accounts) {
    ...
    it("should freeze buyer1", async function () {
        ...
        shouldFail.reverting(selfToken.transfer(buyer2, 1, {from: buyer1}));

        // buyer2 cannot transfer to buyer1
        shouldFail.reverting(selfToken.transfer(buyer1, 1, {from: buyer2}));

        // buyer1 cannot approve buyer2
        shouldFail.reverting(selfToken.approve(buyer2, 1, {from: buyer1}));

        // buyer1 cannot burn
        shouldFail.reverting(selfToken.burn(1, "", {from: buyer1}));

        // buyer3 cannot transferFrom buyer1
        shouldFail.reverting(selfToken.transferFrom(buyer1, anyone, 1, {from: buyer3}));
    });
    ...
    it("should unfreeze buyer1", async function () {
        ...
        // buyer1 can transfer
        await selfToken.transfer(buyer2, 1, {from: buyer1});

        // buyer2 can transfer to buyer1
        await selfToken.transfer(buyer1, 1, {from: buyer2});

        // buyer1 can approve buyer2
        await selfToken.approve(buyer2, 1, {from: buyer1});

        // buyer1 can burn
        await selfToken.burn(1, "", {from: buyer1});

        // buyer3 can transferFrom buyer1
        await selfToken.transferFrom(buyer1, anyone, 1, {from: buyer3});
    });
});